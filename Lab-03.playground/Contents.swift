...
func takeInString(string:String){
    print("***\(string)***")
}

takeInString(string: "Hello")
...
//   returned results.
areaOfRectangle(width: 25, height: 35)
...
func areaOfCircle(radius: Int) -> Double{
    let area: Double = 3.14 * Double(radius * radius)
    return area
}

areaOfCircle(radius: 25)
...
func areaOfSquare(outerLength length: Int, outerWidth width: Int, outerCostPerSqFt costPerSqFt: Double) -> Double{
    return(Double(length) * Double(width) * costPerSqFt)
}

areaOfSquare(outerLength: 32, outerWidth: 64, outerCostPerSqFt: 0.10)
...
//I had a bit of a hard time following exactly what this last question was asking. I don't think I am returning a tuple correctly.
